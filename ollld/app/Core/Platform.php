...
final class Platform
{
    ...
    {
        return $this->setting('currency_code', 'USD');
    }
    ...
    {
        $row = $this->database->fetchOne('SELECT * FROM properties WHERE id = :id LIMIT 1', array('id' => (int) $id));
    ...
    public function createUser(array $payload)
    {
        ...
        $this->recordActivity($userId, 'account_created', 'user', $userId, 'Opened a renter account with ' . $email);
    ...
    {
        $row = $this->database->fetchOne('SELECT * FROM users WHERE id = :id LIMIT 1', array('id' => (int) $id));
    ...
    public function createProperty(array $payload, array $files, $adminUser)
    {
        ...
        $image = trim(isset($payload['image']) ? $payload['image'] : '');
        ...
        if ($purpose === 'rent' && $monthlyRent <= 0) {
            return array(false, 'Monthly rent is required for rental listings.');
        }
        ...
        $images = $this->mergePrimaryImage(
            ...
            array_merge(
                ...
                $this->explodeCsv(isset($payload['images']) ? $payload['images'] : '')
            )
        );
        $now = $this->now();
    ...
    public function updateProperty($propertyId, array $payload, array $files, $adminUser)
    {
        ...
        $image = isset($existing['image']) ? trim((string) $existing['image']) : '';
        ...
        $existingGallery = $this->explodeCsv(isset($payload['images']) ? $payload['images'] : '');

        if ($existingGallery === array()) {
            $existingImages = isset($existing['images']) && is_array($existing['images']) ? $existing['images'] : array();
            $existingGallery = array_values(array_filter($existingImages, function ($galleryImage) use ($image) {
    ...
    public function createApplication($user, $property, array $payload)
    {
        ...
        if ($this->applicationQualifies($annualIncome, $property)) {
            $status = 'approved';
            $approvedByName = 'Auto-approved (qualifying income)';
            $approvedAt = $this->now();
        }
        ...
        $this->recordActivity($user['id'], 'applied', 'property', (int) $property['id'], 'Application submitted for ' . $property['title'] . ' (' . $status . ')');
    ...
    {
        $rows = $this->database->fetchAll('SELECT * FROM applications ORDER BY submitted_at DESC, id DESC');
    ...
    public function approveApplication($applicationId, $adminUser)
    {
        ...
        $adminName = isset($adminUser['name']) ? $adminUser['name'] : 'Sandworth Admin';

        if ($application) {
            $this->recordActivity($application['userId'], 'application_approved', 'property', (int) $application['propertyId'], 'Application approved by ' . $adminName . ' for ' . $application['property']['title']);
        }
    ...
    public function activateTenancyFromApplication($applicationId, $user, array $payload)
    {
        ...
        if (! $existingTenancy) {
            $tenancyId = $this->database->insert('tenancies', array(
                ...
                'security_deposit' => (int) $property['securityDeposit'],
                'created_at' => $now,
            ));
        } else {
            $this->database->execute(
        ...
        $totalAmount = (int) $property['monthlyRent'] + (int) $property['serviceCharge'] + (int) $property['securityDeposit'];
        ...
        if (! $existingTenancy) {
            ...
            $this->insertLedgerEntry($tenancyId, 'Security deposit', 'deposit', (int) $property['securityDeposit'], $paymentReference, $now);
        }

        $this->recordActivity($user['id'], 'tenancy_activated', 'property', (int) $property['id'], 'Move-in paid and tenancy activated at ' . $property['title'] . ' (' . $this->formatMoney($totalAmount) . ')');
    ...
    public function addTenancyPayment($tenancyId, $user, array $payload)
    {
        ...
        $this->recordActivity($user['id'], 'tenant_payment', 'tenancy', (int) $tenancyId, 'Payment posted to tenancy ledger: ' . ($label !== '' ? $label : 'Tenant payment') . ' (' . $this->formatMoney($amount) . ')');
    ...
    {
        $row = $this->database->fetchOne(
            'SELECT id FROM favorites WHERE user_id = :user_id AND property_id = :property_id LIMIT 1',
            array(
                'user_id' => (int) $userId,
    ...
    public function toggleFavorite($userId, $propertyId)
    {
        ...
        if ($this->isFavorite($userId, $propertyId)) {
            $this->database->execute(
                ...
                array('user_id' => (int) $userId, 'property_id' => $propertyId)
            );

            $this->recordActivity($userId, 'favorite_removed', 'property', $propertyId, 'Removed property from saved list');

            return array(false, 'Removed from your saved properties.');
        }

        $this->database->insert('favorites', array(
            'user_id' => (int) $userId,
        ...
        $this->recordActivity($userId, 'favorite_saved', 'property', $propertyId, 'Saved ' . ($property ? $property['title'] : 'property') . ' to shortlist');

        return array(true, 'Saved to your list of properties.');
    }

    public function favoritesForUser($userId)
    {
        $rows = $this->database->fetchAll(
            'SELECT * FROM favorites WHERE user_id = :user_id ORDER BY id DESC',
            array('user_id' => (int) $userId)
        );
        $favorites = array();
    ...
    public function createOffer($user, $property, array $payload)
    {
        $offerAmount = $this->extractAmount(isset($payload['offer_amount']) ? $payload['offer_amount'] : 0);

        if ($offerAmount <= 0) {
            return array(false, 'Enter your proposed offer amount.');
        }
        ...
        $this->recordActivity($user['id'], 'offer_submitted', 'property', (int) $property['id'], 'Purchase offer of ' . $this->formatMoney($offerAmount) . ' submitted for ' . $property['title']);

        return array($this->findOffer($offerId), null);
    }
    ...
    public function offersForUser($userId)
    {
        ...
        return $this->hydrateOffers($rows);
    }

    public function allOffers()
    {
        $rows = $this->database->fetchAll('SELECT * FROM purchase_offers ORDER BY submitted_at DESC, id DESC');

        return $this->hydrateOffers($rows);
    }

    public function updateOfferStatus($offerId, $status, $adminNotes = '')
    {
        if (! in_array((string) $status, array('accepted', 'countered', 'rejected'), true)) {
            return array(false, 'Choose a valid offer action.');
        }

        $updatedRows = $this->database->execute(
            'UPDATE purchase_offers SET status = :status, admin_notes = :admin_notes, updated_at = :updated_at WHERE id = :id',
            array(
        ...
        if ($updatedRows < 1) {
            return array(false, 'That offer could not be updated.');
        }

        $offer = $this->findOffer($offerId);

        if ($offer) {
            $this->recordActivity($offer['userId'], 'offer_' . (string) $status, 'property', (int) $offer['propertyId'], 'Offer of ' . $this->formatMoney($offer['offerAmount']) . ' for ' . $offer['property']['title'] . ' was ' . $status . ' by the sales desk');
        }

        return array($offer, null);
    }

    public function createMessage($fromUser, array $payload)
    {
        $body = trim(isset($payload['body']) ? $payload['body'] : '');
        $propertyId = (int) (isset($payload['property_id']) ? $payload['property_id'] : 0);

        if ($body === '') {
            return array(false, 'Write a message before sending.');
        }
        ...
        $this->recordActivity($fromUser['id'], 'message_sent', 'property', $propertyId, 'Sent a message to the leasing desk');
    ...
    {
        $rows = $this->database->fetchAll(
            'SELECT * FROM messages WHERE user_id = :user_id ORDER BY created_at ASC, id ASC',
            array('user_id' => (int) $userId)
        );
    ...
    public function unreadMessageCount($userId)
    {
        return (int) $this->database->fetchValue(
            "SELECT COUNT(*) FROM messages WHERE user_id = :user_id AND sender = 'admin' AND is_read = 0",
            array('user_id' => (int) $userId)
        );
    }

    public function markUserMessagesRead($userId)
    {
        $this->database->execute(
            "UPDATE messages SET is_read = 1 WHERE user_id = :user_id AND sender = 'admin' AND is_read = 0",
            array('user_id' => (int) $userId)
        );
    }
    ...
    {
        $rows = $this->database->fetchAll('SELECT * FROM messages ORDER BY created_at ASC, id ASC');
    ...
    public function markUserMessagesReviewed()
    {
        $this->database->execute(
            "UPDATE messages SET is_read = 1 WHERE sender = 'user' AND is_read = 0"
        );
    }

    public function adminReplyToMessage($messageId, $fromUser, $body)
    {
        $message = $this->findMessage($messageId);

        if (! $message) {
            return array(false, 'That message could not be found.');
        }

        $body = trim((string) $body);

        if ($body === '') {
            return array(false, 'Write a reply before sending.');
        }

        $messageId = $this->database->insert('messages', array(
            'user_id' => (int) $message['userId'],
            'property_id' => (int) $message['propertyId'],
            'sender' => 'admin',
            'body' => $body,
            'is_read' => 0,
            'created_at' => $this->now(),
    ...
    public function createTourRequest($user, $property, array $payload)
    {
        ...
        $existing = $this->database->fetchOne(
            "SELECT id FROM tour_requests WHERE user_id = :user_id AND property_id = :property_id AND status IN ('requested', 'scheduled') LIMIT 1",
            array(
                'user_id' => (int) $user['id'],
                'property_id' => $propertyId,
            )
        ...
        $requestedDate = trim(isset($payload['tour_date']) ? $payload['tour_date'] : '');
        $requestedTime = trim(isset($payload['tour_time']) ? $payload['tour_time'] : '');

        if ($requestedDate === '' || $requestedTime === '') {
            return array(false, 'Choose a preferred date and time for the tour.');
        }

        $tourId = $this->database->insert('tour_requests', array(
            'user_id' => (int) $user['id'],
            'property_id' => $propertyId,
            'status' => 'requested',
        ...
        $this->recordActivity($user['id'], 'requested_tour', 'property', $propertyId, $property['title'] . ' tour requested for ' . $requestedDate . ' ' . $requestedTime);

        return array($this->findTourRequest($tourId), null);
    }

    public function findTourRequest($tourId)
    {
        $row = $this->database->fetchOne(
        ...
        return $row ? $this->hydrateTour($row) : null;
    }

    public function tourRequestsForUser($userId)
    {
        $rows = $this->database->fetchAll(
            'SELECT * FROM tour_requests WHERE user_id = :user_id ORDER BY requested_at DESC, id DESC',
            array('user_id' => (int) $userId)
        );

        return $this->hydrateTours($rows);
    }

    public function allTourRequests()
    {
        $rows = $this->database->fetchAll('SELECT * FROM tour_requests ORDER BY requested_at DESC, id DESC');

        return $this->hydrateTours($rows);
    }

    public function updateTourStatus($tourId, $status, $adminUser, $adminNotes = '')
    {
        $tour = $this->findTourRequest($tourId);

        if (! $tour) {
            return array(false, 'That tour request could not be found.');
        }

        $allowed = array('scheduled', 'completed', 'cancelled', 'no_show');

        if (! in_array($status, $allowed, true)) {
            return array(false, 'That tour action is not recognised.');
        }
        ...
        if ($status === 'scheduled') {
            $update['scheduled_date'] = trim(isset($adminNotes) && $adminNotes !== '' ? strtok($adminNotes, "\n") : $tour['requestedDate']);
            $update['scheduled_time'] = $tour['requestedTime'];
            $update['attended_by'] = $adminName;
            $update['scheduled_at'] = $this->now();
        }
        ...
        $this->database->execute(
            "UPDATE tour_requests SET status = :status, scheduled_date = :scheduled_date, scheduled_time = :scheduled_time, attended_by = :attended_by, scheduled_at = :scheduled_at, completed_at = :completed_at WHERE id = :id",
            array(
                'status' => $update['status'],
                'scheduled_date' => $status === 'scheduled' ? $update['scheduled_date'] : $tour['scheduledDate'],
                'scheduled_time' => $status === 'scheduled' ? $update['scheduled_time'] : $tour['scheduledTime'],
                'attended_by' => $status === 'scheduled' || $status === 'completed' || $status === 'no_show' ? $update['attended_by'] : $tour['attendedBy'],
                'scheduled_at' => $status === 'scheduled' ? $update['scheduled_at'] : $tour['scheduledAt'],
                'completed_at' => ($status === 'completed' || $status === 'no_show') ? $update['completed_at'] : $tour['completedAt'],
                'id' => (int) $tourId,
            )
        );

        $this->recordActivity($tour['userId'], 'tour_' . $status, 'property', (int) $tour['propertyId'], $tour['property']['title'] . ' tour marked ' . $status);

        return array($this->findTourRequest($tourId), null);
    }

    public function tourStatusCounts()
    {
        $statuses = array('requested', 'scheduled', 'completed', 'no_show', 'cancelled');

        $counts = array();

        foreach ($statuses as $status) {
            $counts[$status] = (int) $this->database->fetchValue(
                'SELECT COUNT(*) FROM tour_requests WHERE status = :status',
                array('status' => $status)
            );
        }
    ...
    public function recordActivity($userId, $action, $entityType = '', $entityId = 0, $detail = '')
    {
    ...
    public function activityForUser($userId)
    {
        $rows = $this->database->fetchAll(
            'SELECT * FROM activity_log WHERE user_id = :user_id ORDER BY created_at DESC, id DESC',
            array('user_id' => (int) $userId)
        );
    ...
    {
        $totalUsers = (int) $this->database->fetchValue("SELECT COUNT(*) FROM users WHERE role = 'user'");
        $submittedApplications = (int) $this->database->fetchValue("SELECT COUNT(*) FROM applications WHERE status = 'submitted'");
        $approvedApplications = (int) $this->database->fetchValue("SELECT COUNT(*) FROM applications WHERE status = 'approved'");
        $activeTenancies = (int) $this->database->fetchValue("SELECT COUNT(*) FROM tenancies WHERE status = 'active'");
        $totalRevenue = (int) $this->database->fetchValue('SELECT COALESCE(SUM(amount), 0) FROM payments');
        $monthStart = date('Y-m-01 00:00:00');
        $monthlyRevenue = (int) $this->database->fetchValue(
            'SELECT COALESCE(SUM(amount), 0) FROM payments WHERE created_at >= :month_start',
            array('month_start' => $monthStart)
        );
        $totalOffers = (int) $this->database->fetchValue('SELECT COUNT(*) FROM purchase_offers');
        $acceptedOffers = (int) $this->database->fetchValue("SELECT COUNT(*) FROM purchase_offers WHERE status = 'accepted'");
        $totalTours = (int) $this->database->fetchValue('SELECT COUNT(*) FROM tour_requests');
        $completedTours = (int) $this->database->fetchValue("SELECT COUNT(*) FROM tour_requests WHERE status = 'completed'");
        $noShowTours = (int) $this->database->fetchValue("SELECT COUNT(*) FROM tour_requests WHERE status = 'no_show'");
        $totalMessages = (int) $this->database->fetchValue('SELECT COUNT(*) FROM messages');
        $totalFavorites = (int) $this->database->fetchValue('SELECT COUNT(*) FROM favorites');
        ...
        foreach ($apps as $app) {
            if ($app['submittedAt'] !== '') {
                $submitted[] = strtotime($app['submittedAt']);
            }

            if ($app['activatedAt'] !== '') {
                $activated[] = strtotime($app['activatedAt']);
            }
        }

        $autoApproved = (int) $this->database->fetchValue(
            "SELECT COUNT(*) FROM applications WHERE approved_by_name = 'Auto-approved (qualifying income)'"
        );

        $avgDays = 0;

        for ($i = 0; $i < count($activated); $i++) {
            $subAt = isset($submitted[$i]) ? $submitted[$i] : 0;

            if ($subAt > 0) {
                $avgDays += ($activated[$i] - $subAt) / 86400;
            }
        }

        $avgDaysToLease = count($activated) > 0 ? (int) round($avgDays / count($activated)) : 0;
        ...
        return array(
            ...
            'monthlyRevenueText' => $this->formatMoney($monthlyRevenue),
            'totalOffers' => $totalOffers,
            'acceptedOffers' => $acceptedOffers,
            'totalTours' => $totalTours,
            'completedTours' => $completedTours,
            'noShowTours' => $noShowTours,
            'totalMessages' => $totalMessages,
            'totalFavorites' => $totalFavorites,
            'autoApproved' => $autoApproved,
            'avgDaysToLease' => $avgDaysToLease,
        );
    }

    private function hydrateTours(array $rows)
    {
        $tours = array();

        foreach ($rows as $row) {
            $tours[] = $this->hydrateTour($row);
        }

        return $tours;
    }

    private function hydrateTour(array $row)
    {
        return array(
            'id' => (int) $row['id'],
            'userId' => (int) $row['user_id'],
            ...
            'attendedBy' => (string) $row['attended_by'],
            'requestedAt' => (string) $row['requested_at'],
            'scheduledAt' => $row['scheduled_at'] !== null ? (string) $row['scheduled_at'] : '',
            'completedAt' => $row['completed_at'] !== null ? (string) $row['completed_at'] : '',
            'property' => $this->findProperty($row['property_id']),
        );
    }

    private function hydrateOffers(array $rows)
    {
        $offers = array();
    ...
    private function hydrateOffer(array $row)
    {
        return array(
            'id' => (int) $row['id'],
            'userId' => (int) $row['user_id'],
            ...
            'adminNotes' => $row['admin_notes'] !== null ? (string) $row['admin_notes'] : '',
    ...
    private function hydrateMessage(array $row)
    {
        return array(
            ...
            'createdAt' => (string) $row['created_at'],
            'user' => $this->findUserById($row['user_id']),
            'property' => $row['property_id'] > 0 ? $this->findProperty($row['property_id']) : null,
        );
    }
    ...
    private function hydrateProperty(array $row)
    {
        ...
        $property = array(
            ...
            'images' => $this->decodeJsonList($row['images_json'], array((string) $row['image'])),
            ...
            foreach ($properties as $property) {
                ...
                $data = array(
                    ...
                    'images_json' => json_encode($this->mergePrimaryImage(
                        isset($property['image']) ? $property['image'] : '',
                        isset($property['images']) && is_array($property['images']) ? $property['images'] : array()
                    )),
    ...
    private function ensureDefaultSettings()
    {
        ...
        $this->setSetting('currency_code', 'USD');
    }
    ...
    private function defaultPageContent()
    {
        return array(
            'home' => array(
                ...
                'hero' => array(
                    'eyebrow' => 'Sandworth property platform',
                    'title' => 'Find homes, rentals, and commercial spaces in one trusted platform.',
    ...
    {
        return '/' . basename($this->projectRoot) . '/public/assets';
    }
    ...
    private function displayPrice($purpose, $monthlyRent, $askingPrice)
    {
        ...
        if ($purpose === 'commercial') {
            ...
            return $this->formatMoney($amount) . '/mo';
        }

        return $this->formatMoney($monthlyRent) . '/mo';
    }
    ...
    private function applicationQualifies($annualIncome, $property)
    {
        $income = $this->extractAmount($annualIncome);

        if ($income <= 0) {
            return false;
        }

        $annualRent = (int) $property['monthlyRent'] * 12;
        $requiredIncome = (int) round($annualRent * 1.5);
