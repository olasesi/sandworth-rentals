...
$applicationLegalFee = $application ? app_legal_fee_amount($application['property']['monthlyRent']) : 0;
$applicationCautionDeposit = $application ? app_caution_deposit_amount($application['property']['securityDeposit']) : 0;
$applicationMoveInTotal = $application ? app_move_in_total($application['property']['monthlyRent'], $application['property']['serviceCharge'], $applicationCautionDeposit) : 0;
?>
            ...
            <div class="detail-card">
                ...
                <div style="display:grid; gap:10px; font-size:.875rem">
                    ...
                    <div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid var(--line-light)">
                        ...
                        <strong><?= htmlspecialchars(app_currency($applicationLegalFee), ENT_QUOTES, 'UTF-8') ?></strong>
                    </div>
                    <div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid var(--line-light)">
                        <span style="color:var(--muted)">Caution deposit</span>
                        <strong><?= htmlspecialchars(app_currency($applicationCautionDeposit), ENT_QUOTES, 'UTF-8') ?></strong>
                    </div>
                    <div style="display:flex; justify-content:space-between; padding:10px 0; font-size:1rem">
                        ...
                        <strong style="color:var(--primary); font-size:1.1rem">
                            <?= htmlspecialchars(app_currency($applicationMoveInTotal), ENT_QUOTES, 'UTF-8') ?>
                        </strong>
                    </div>
                </div>
        ...
        <aside>
            <div class="detail-card">
                ...
                <div style="font-size:.875rem; display:grid; gap:8px">
                    ...
                    <div style="display:flex; justify-content:space-between">
                        <span style="color:var(--muted)">Legal fee (10%)</span>
                        <strong><?= htmlspecialchars(app_currency(app_legal_fee_amount($tenancy['monthlyRent'])), ENT_QUOTES, 'UTF-8') ?></strong>
                    </div>
                    <div style="display:flex; justify-content:space-between">
                        <span style="color:var(--muted)">Caution deposit</span>
                        <strong><?= htmlspecialchars(app_currency(app_caution_deposit_amount($tenancy['securityDeposit'])), ENT_QUOTES, 'UTF-8') ?></strong>
                    </div>
                </div>
            </div>
        </aside>
